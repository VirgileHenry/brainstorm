...
pub struct SpellSubtype {
    pub spell_subtype: mtg_data::SpellType,
...
impl SpellSubtype {
    pub fn all() -> impl Iterator<Item = Self> {
        mtg_data::SpellType::all().map(|spell_subtype| SpellSubtype {
            spell_subtype,
            #[cfg(feature = "spanned_tree")]
...
impl AbilityTreeNode for SpellSubtype {
    fn node_id(&self) -> usize {
        crate::ability_tree::NodeKind::MtgData(MtgDataNodeKind::SpellSubtypeIdMarker).id()
    }

    fn children(&self) -> arrayvec::ArrayVec<&dyn AbilityTreeNode, MAX_CHILDREN_PER_NODE> {
        let mut children = arrayvec::ArrayVec::new_const();
        let child_id = NodeKind::MtgData(MtgDataNodeKind::ObjectKind(super::ObjectKind::InstantSorcerySubtype(*self))).id();
...
#[cfg(feature = "parser")]
impl crate::utils::DummyInit for SpellSubtype {
    fn dummy_init() -> Self {
        Self {
            spell_subtype: mtg_data::SpellType::Adventure,
            #[cfg(feature = "spanned_tree")]
            span: Default::default(),
        }
    }
