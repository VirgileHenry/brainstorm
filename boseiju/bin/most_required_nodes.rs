...
fn main() {
    ...
    for (i, card) in cards.iter().enumerate() {
        ...
        let (token, context) = match parse(&tokens) {
            Err(parser::ParserError::UnexpectedToken { found, context, .. }) => (found, context),
            _ => continue,
        };

        let node = most_required_nodes.entry(token.name).or_default();
        node.count += 1;
        node.as_in.push(context);
    }
    ...
    for (token, node) in most_required_tokens.iter().take(SHOWN_NODES) {
        println!("{token} ({} cards) as in:", node.count);
