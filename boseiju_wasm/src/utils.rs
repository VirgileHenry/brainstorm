...
pub fn lexer_error_to_json(error: boseiju_lexer::LexerError) -> String {
    match error {
        boseiju_lexer::LexerError::NoTokenMatch { start, end, tokens } => {
...
pub fn parser_error_to_json(error: boseiju_parser::ParserError) -> String {
    match error {
        boseiju_parser::ParserError::AmbiguousCandidates => {
            let message = "Multiple possible ways of merging the tokens !";
            format!("{{\"start\":0,\"end\":0,\"message\":\"{message}\"}}")
        }
        boseiju_parser::ParserError::FailedToApplyRule { merge_error, for_rule } => {
            let message = format!("Failed to apply rule: {merge_error} (rule defined at {for_rule})");
            let message = message.replace('"', "\\\"");
            let message = message.replace('\n', "\\n");
            format!("{{\"start\":0,\"end\":0,\"message\":\"{message}\"}}")
        }
        boseiju_parser::ParserError::InvalidEarleyTable => {
            let message = "Invalid Earley table, something went VERY wrong :)";
            let message = message.replace('"', "\\\"");
            let message = message.replace('\n', "\\n");
            format!("{{\"start\":0,\"end\":0,\"message\":\"{message}\"}}")
        }
        boseiju_parser::ParserError::UnexpectedToken { found, expecting, .. } => {
            let mut message = format!("Unexpected token {} at position {}", found.name, found.position);
            message.push_str(&format!("\nExpecting one of:"));
            for expecting in expecting.iter().take(10) {
                let node_name = <boseiju_parser::ParserNode as idris::Idris>::name_from_id(expecting.expected);
                message.push_str(&format!("\n - token \"{node_name}\" to create nodes"));
                for (i, (for_node, for_rule)) in expecting.for_nodes.iter().enumerate() {
                    let node_name = <boseiju_parser::ParserNode as idris::Idris>::name_from_id(*for_node);
                    if i == 0 {
