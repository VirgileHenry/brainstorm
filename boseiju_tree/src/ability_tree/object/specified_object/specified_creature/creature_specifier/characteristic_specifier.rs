...
impl crate::Node for CreatureCharacteristicSpecifier {
    ...
    fn children(&self) -> arrayvec::ArrayVec<&dyn Node, MAX_CHILDREN_PER_NODE> {
        ...
        match self {
            Self::Power(child) => children.push(child as &dyn Node),
...
impl boseiju_span::Spanned for CreatureCharacteristicSpecifier {
    fn span(&self) -> boseiju_span::Span {
        match self {
            Self::Power(child) => child.span(),
            Self::KeywordAbility(child) => child.span(),
        }
    }
}

impl Default for CreatureCharacteristicSpecifier {
    fn default() -> Self {
        Self::Power(Default::default())
    }
}
