...
impl Node for Ability {
    ...
    fn children(&self) -> arrayvec::ArrayVec<&dyn Node, MAX_CHILDREN_PER_NODE> {
        ...
        match self {
            Self::AbilityWord(ability) => abilities.push(ability as &dyn Node),
            Self::KeywordAbility(ability) => abilities.push(ability as &dyn Node),
            Self::Written(ability) => abilities.push(ability as &dyn Node),
        };
        abilities
    }
...
impl boseiju_span::Spanned for Ability {
...
impl Node for WrittenAbility {
    ...
    fn children(&self) -> arrayvec::ArrayVec<&dyn Node, MAX_CHILDREN_PER_NODE> {
        let mut abilities = arrayvec::ArrayVec::new_const();
        match self {
            Self::Spell(ability) => abilities.push(ability as &dyn Node),
            Self::Activated(ability) => abilities.push(ability as &dyn Node),
            Self::Triggered(ability) => abilities.push(ability as &dyn Node),
            Self::Static(ability) => abilities.push(ability as &dyn Node),
        };
        abilities
    }
...
#[cfg(feature = "spanned_tree")]
impl boseiju_span::Spanned for WrittenAbility {
    fn span(&self) -> boseiju_span::Span {
        match self {
...
impl Default for WrittenAbility {
    fn default() -> Self {
        Self::Spell(Default::default())
    }
}
...
impl Default for KeywordAbility {
    fn default() -> Self {
        Self {
...
impl Node for AbilityWordAbility {
    ...
    fn children(&self) -> arrayvec::ArrayVec<&dyn Node, MAX_CHILDREN_PER_NODE> {
        let mut abilities = arrayvec::ArrayVec::new_const();
        abilities.push(&self.word as &dyn Node);
        abilities.push(&self.ability as &dyn Node);
        abilities
    }
...
#[cfg(feature = "spanned_tree")]
impl boseiju_span::Spanned for AbilityWordAbility {
    fn span(&self) -> boseiju_span::Span {
        self.span
    }
}

impl Default for AbilityWordAbility {
    fn default() -> Self {
        Self {
