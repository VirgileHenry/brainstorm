...
impl super::LayoutImpl for DoubleFaced {
    ...
    fn from_raw_card(raw_card: &mtg_cardbase::Card) -> Result<Self, String> {
        ...
        let (first_face, second_face) = match faces.as_slice() {
            ...
            _other => return Err(format!("Transform card should have exactly two faces! o(>< )o")),
        };
...
impl AbilityTreeNode for DoubleFaced {
    fn node_id(&self) -> usize {
        use crate::ability_tree::tree_node::LayoutNodeKind;
        use idris::Idris;

        crate::ability_tree::NodeKind::Layout(LayoutNodeKind::Transform).id()
    }
    ...
    fn display(&self, out: &mut crate::utils::TreeFormatter<'_>) -> std::io::Result<()> {
        use std::io::Write;
        write!(out, "double faced token layout")?;
