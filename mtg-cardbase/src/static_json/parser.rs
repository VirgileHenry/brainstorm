...
macro_rules! expect_fraction {
    ($parser:ident, $num:ident, $e:ident) => {{
        ...
        loop {
            if $parser.is_eof() {
                result = unsafe { json::number::Number::from_parts_unchecked(true, $num, $e) };
                break;
            }
            let ch = $parser.read_byte();
...
impl<'a> Parser<'a> {
    ...
    // Boring
    fn read_hexdec_digit(&mut self) -> Result<u16, String> {
    ...
    // Boring
    fn read_hexdec_codepoint(&mut self) -> Result<u16, String> {
    ...
    fn read_codepoint(&mut self) -> Result<(), String> {
        let mut buf = [0; 4];
        ...
        let unicode = match char::try_from(codepoint as u32) {
            ...
            Err(_) => {
                ...
                {
                    Some(Ok(code)) => code,
                    _ => return Err(format!("Failed UTF-8 parsing")),
                }
            }
        };
    ...
    fn read_complex_string<'b>(&mut self, start: usize) -> Result<&'b str, String> {
    ...
    fn read_big_number(&mut self, mut num: u64) -> Result<json::number::Number, String> {
        ...
        loop {
            if self.is_eof() {
                return Ok(unsafe { json::number::Number::from_parts_unchecked(true, num, e) });
            }
        ...
        Ok(unsafe { json::number::Number::from_parts_unchecked(true, num, e) })
    }
    ...
    fn expect_exponent(&mut self, num: u64, big_e: i16) -> Result<json::number::Number, String> {
        ...
        Ok(unsafe {
            json::number::Number::from_parts_unchecked(true, num, big_e.saturating_add(e * sign))
        })
    }
    ...
    fn parse(&mut self) -> Result<super::StaticJsonValue, String> {
        ...
        'parsing: loop {
            let mut value = match ch {
                ...
                b'"' => super::StaticJsonValue::String(expect_string!(self)),
                b'0' => super::StaticJsonValue::Number(allow_number_extensions!(self)),
                b'1'..=b'9' => super::StaticJsonValue::Number(expect_number!(self, ch)),
                b'-' => {
                    let ch = expect_byte!(self);
...
#[inline]
pub fn parse(source: &str) -> Result<super::StaticJsonValue, String> {
