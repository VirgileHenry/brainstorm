...
impl<'r> EarleyRow<'r> {
    ...
    fn start_row(rules: &'static rule_map::RuleMap) -> EarleyRow<'static> {
        ...
        let target_node_id = ParserNode::AbilityTree {
        ...
        if let Some(rules) = rules.get_rules_for_token(target_node_id) {
            for rule in rules {
...
pub fn parse_impl(tokens: &[boseiju_lexer::Token]) -> Result<boseiju_tree::AbilityTree, ParserError> {
    use idris::Idris;

    if tokens.is_empty() {
        return Ok(boseiju_tree::AbilityTree::empty());
    }
    ...
    match completed_items.as_slice() {
        ...
        &[] => Err(ParserError::from_earley_table(&earley_table, tokens)),
