...
fn main() {
    ...
    for (i, card) in cards.iter().enumerate() {
        ...
        let oracle_text = boseiju_lexer::preprocess(card_name.as_str(), oracle_text);
        let tokens = match boseiju_lexer::lex(&oracle_text) {
            Ok(tokens) => tokens,
            Err(_) => continue,
        };
        let _ = match boseiju_parser::parse_ability_tree(&tokens) {
            Ok(tree) => tree,
            Err(_) => continue,
        };
