...
fn main() -> std::io::Result<()> {
    ...
    let result: Vec<boseiju_tree::Card> = chunks
        .par_iter()
        .zip(bars.par_iter())
        .flat_map(|(chunk, pb)| {
            let total = chunk.len();
            let mut last_shown_percentage = 0usize;
            let mut local: Vec<boseiju_tree::Card> = Vec::with_capacity(total);
            ...
            for (i, card) in chunk.iter().enumerate() {
                ...
                if let Ok(parsed) = boseiju_parser::parse_card(*card) {
                    local.push(parsed);
                }
            }
    ...
    /* Smol: keep only 20 cards (yeah, we parsed them all to throw them away, sue me) */
    let result: Vec<_> = result.into_iter().take(20).collect();
