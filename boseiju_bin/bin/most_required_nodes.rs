...
fn main() {
    ...
    chunks.par_iter().zip(bars.par_iter()).for_each(|(chunk, pb)| {
        ...
        for (i, card) in chunk.iter().enumerate() {
            ...
            let preprocessed = boseiju_lexer::preprocess(card.name.as_str(), oracle_text);
            let tokens = match boseiju_lexer::lex(&preprocessed) {
                Ok(preprocessed) => preprocessed,
                Err(_) => continue,
            };
            let token = match boseiju_parser::parse_ability_tree(&tokens) {
                Err(boseiju_parser::ParserError::UnexpectedToken { found, .. }) => found,
                _ => continue,
            };
